...
public class RegisterController {
    ...
    public void initialize() {
        ...
        registerSpinner = createInlineSpinner();
    ...
    private boolean isDigitsOnly(String phone) {
        return phone != null && !phone.isBlank() && phone.chars().allMatch(Character::isDigit);
    }

    private void setRegisterPending(boolean pending) {
        registerPending = pending;
        if (registerButton != null) {
            registerButton.setDisable(pending);
            registerButton.setText(pending ? "Creating Account..." : "Create Account");
            registerButton.setGraphic(pending ? registerSpinner : null);
            registerButton.setContentDisplay(ContentDisplay.RIGHT);
            registerButton.setGraphicTextGap(8);
        }
    }
